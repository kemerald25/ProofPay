...
// This is a public-facing key, safe to be in browser code
...
export default function EscrowDetailPage({ params }: { params: { id: string } }) {
  ...
  const chatStorageKey = `${CHAT_STORAGE_KEY_PREFIX}${params.id}`;
    ...
    async function fetchEscrow() {
      ...
      if (error || !data) {
        ...
        toast({
            variant: 'destructive',
            title: 'Error',
            description: `Could not load escrow ${params.id}.`,
        });
      } else {
        setEscrow(data);
        setPhone(data.buyer_phone); // Default to buyer
      }
      setLoading(false);
    }
    fetchEscrow();
